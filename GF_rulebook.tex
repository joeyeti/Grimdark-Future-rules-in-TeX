\documentclass[9pt, a4paper]{extarticle}            %% A4 paper size, default font size 9 pts.
...
\usepackage{eso-pic, tikz}                          %% eso-pic provides tools for including graphics on full paper size, tikz package is used for graphics
...
\setlength{\footskip}{1.45cm}        %% longer distance between text and Page numbering / foot

\usepackage{fancyhdr}           %% change the style of the footer - increase page numbering font size
\pagestyle{fancy}    
...
\fancypagestyle{plain}{     %% % change plain style to the same style (eg. used on chapter pages - here on Page 2)
  \fancyhf{}
  \fancyfoot[C]{\fontsize{12}{14} \selectfont \thepage}
...
%% \setcounter{tocdepth}{1}             %% to be included once TOC style is sorted out

%% ---- Page 01 ---- %%

\begin{document}

\vfill\null                 %% included her as something has to be on the page for the Cover to render

\AddToShipoutPictureBG*{\includegraphics[width=\paperwidth,height=\paperheight]{GF_rulebook_page_01.png}}
...
\pagenumbering{arabic}\setcounter{page}{2}      %% start page numbering on here on Page 2, thus skip Page 1 / Cover

\AddToShipoutPictureBG{\includegraphics[width=\paperwidth,height=\paperheight]{GF_rulebook_background.png}}
                                %% above is the background picture used for the rest of the pages
